...
class BusinessOwnerRepository {
  ...
  Future<void> addBusinessUser(BusinessOwner businessOwner) async {
    try {
  ...
  Future<void> updateBusinessUser(String userId, BusinessOwner businessOwner) async {
    try {
  ...
  Future<List<BusinessOwner>> getAllBusinessUsers() async {
    try {
      ...
      return snapshot.docs.map((doc) => BusinessOwner.fromDocument(doc)).toList();
    } catch (e) {
  ...
  Future<BusinessOwner> getBusinessUser(String userId) async {
    try {
      DocumentSnapshot snapshot = await _businessOwnerCollection.doc(userId).get();
      if (snapshot.exists) {
